...
- Template rules are kid-scoped: drop rules carry `src-address-list=kids-devices`; the script syncs that list every cycle from kid-control devices bound to `hook:*` users (MAC → DHCP lease → IP), marking its entries `kc:auto` (manual entries untouched)
- Template DNS layer is a SENSOR, not a blocker: always-on `dns/static type=FWD match-subdomain=yes address-list=blocked-<id>` harvesters (tag `tpl:<id>`, ROS ≥ 7.8) feed every resolved subdomain IP into the lists while answering normally — covers rotating CDN subdomains apex-FQDN resolution misses. NEVER tag harvesters `hook:` (the toggle cycle would flip them) and NEVER generate NXDOMAIN — it is LAN-wide and poisons the router's own FQDN resolution; the generated script removes legacy NXDOMAIN entries and adds missing kids scope (migration)
- SNI enforcement: per-domain `tls-host=*<domain>` tcp/443 drop rules share the service's `hook:<id>` tag (toggle together with the main rule); a permanent `tpl:quic-block` rule drops udp/443 for kids so browsers fall back to TCP TLS where SNI is visible. Because several filter rules share one hook tag, existence guards in the generated script must be SPECIFIC (`&& dst-address-list=...` / `&& tls-host=...`), never just the comment
