...
- **drop-правило** с тегом `hook:<id>`, **ограниченное детскими устройствами** (`src-address-list=kids-devices`) — блокировки не задевают остальную сеть; резолвы взрослых устройств тоже пополняют листы, усиливая блок для детей.

NXDOMAIN-записей шаблоны не создают: их нельзя скоупить по источнику (валили резолв всей LAN) и они отравляли собственный резолвер роутера. DNS здесь — сенсор, а не блокировщик; блокирует только фаервол.
