...
Каталог готовых блокировок (`server/templates.go`) — на каждый сервис FQDN address-list (у игровых дополнительно статические сети: Roblox AS22697, Valve) и drop-правило, **ограниченное детскими устройствами** (`src-address-list=kids-devices`) — блокировки не задевают остальную сеть. DNS-записей шаблоны не создают: NXDOMAIN нельзя скоупить по источнику (валил бы резолв всей LAN), и он отравляет собственный резолвер роутера, опустошая FQDN-листы фаервола.
