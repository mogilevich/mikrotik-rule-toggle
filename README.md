...
Основной экран — пресеты-режимы, секция устройств с паузой, группы сервисов с мастер-тогглами и групповыми таймерами, обратный отсчёт и продление, статус роутера. Синий тумблер у устройства = kid-control активен (режим расписания), красный у сервиса = блокировка. Поддерживается светлая и тёмная тема.

<p align="center">
  <img src="docs/screenshot-templates.png" alt="Шаблоны" width="320">
  &nbsp;&nbsp;
  <img src="docs/screenshot-stats.png" alt="Статистика" width="320">
</p>

Шаблоны — каталог готовых блокировок по группам с импортом в один тап. Аналитика — графики использования таймеров за 1/7/30 дней.
...
Каталог готовых блокировок (`server/templates.go`). На каждый сервис:

- **address-list `blocked-<id>`** — наполняется трёхслойно: апекс-FQDN как база, статические сети у игровых (Roblox AS22697, Valve), и главное — **DNS-харвестинг**: постоянно включённые записи `/ip/dns/static type=FWD match-subdomain=yes address-list=blocked-<id>` (тег `tpl:<id>`, требуют ROS ≥ 7.8). Любой резолв любого сабдомена через DNS роутера отвечает клиенту нормально, но складывает полученные IP в лист — так покрываются ротирующиеся CDN-сабдомены (`rN---sn-*.googlevideo.com` и т.п.), которые апекс-резолв не видит. Апстрим FWD-записей импорт-скрипт берёт из настроек DNS самого роутера.
- **drop-правило по address-list** с тегом `hook:<id>`, **ограниченное детскими устройствами** (`src-address-list=kids-devices`) — блокировки не задевают остальную сеть; резолвы взрослых устройств тоже пополняют листы, усиливая блок для детей.
- **SNI-правила** (`tls-host=*<домен>`, tcp/443) — по одному на домен, тот же тег `hook:<id>`, тоглятся вместе с сервисом. Матчат hostname прямо в TLS ClientHello — режут соединения к IP, которых листы никогда не видели (DoH-обход, свежие CDN-адреса).
- **Блок QUIC** (`tpl:quic-block`, udp/443, всегда включён для kids-devices) — SNI виден только в TCP TLS; без QUIC браузеры откатываются на TCP, где SNI-правила работают.

NXDOMAIN-записей шаблоны не создают: их нельзя скоупить по источнику (валили резолв всей LAN) и они отравляли собственный резолвер роутера. DNS здесь — сенсор, а не блокировщик; блокирует только фаервол. Остаточный обход — ECH (шифрование SNI) + DoH одновременно; на 2026 год встречается редко.

Импорт двухсторонний:
...
2. **В фаервол роутера — автоматически** (скрипт ≥ v22): сервер отдаёт роутеру `templates_hash` — отпечаток сгенерированного скрипта по импортированным в приложение сервисам. Роутер хранит последний применённый хеш и при расхождении (импортирован новый сервис / обновились домены каталога) один раз скачивает `templates.rsc?imported=1` и импортирует. Совпадает — ничего не качает, обычный цикл не замедляется. Скрипт идемпотентен: существующие записи не дублируются, всё новое создаётся `disabled=yes`.

Ручной импорт всего каталога (не только импортированного в приложение):
...
### Контроль «параметр есть, правила нет»

При скане роутер собирает все найденные `hook:`-теги и отправляет их следующим циклом в заголовке `X-Seen-Params`. Параметры, которых нет среди найденных на роутере, UI помечает красным бейджем «нет правила на роутере» (данные отстают на один цикл: после автосоздания правил бейдж гаснет за ~2 минуты).

При обновлении с ранних версий шаблонов миграция выполняется автоматически: существующие шаблонные правила получают `src-address-list=kids-devices`, шаблонные DNS-записи удаляются.

### Детские устройства: address-list `kids-devices`

Единый список «чьи IP детские», на него ссылаются все шаблонные drop-правила. Наполняется автоматически (скрипт ≥ v24, каждый цикл):

1. Берутся устройства kid-control, привязанные к юзерам с именем `hook:*` (то есть к тем, кем управляет панель): `/ip/kid-control/device` → MAC.
2. MAC мапится на IP через DHCP lease (включая reservations офлайн-устройств, обе DHCP-зоны).
3. Список синхронизируется: авто-записи помечены комментарием `kc:auto` (устаревшие удаляются, новые добавляются), **ручные записи не трогаются** — можно добавлять IP и вручную.

Чтобы устройство попало под детские блокировки: привязать его в kid-control (`/ip/kid-control/device add user="hook:<имя>" mac-address=<MAC>`) и завести статический DHCP lease (рекомендуется). Проверка: `/ip/firewall/address-list print where list=kids-devices`.
